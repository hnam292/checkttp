...
import {
  ...
  ScanLine, ShieldCheck, TicketCheck, TriangleAlert,
} from "lucide-react";
...
export default function VoucherTerminal() {
  ...
  useEffect(() => {
    ...
    // Restore browser session state after the client has mounted.
  ...
  async function startScanner() {
    ...
    if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) {
      toast.error("Trình duyệt này không hỗ trợ camera an toàn. Hãy mở trang bằng Safari trên iPhone hoặc Chrome trên Android.");
      return;
    }
    try {
      ...
      const rearCamera = [...cameras].reverse().find((camera) =>
        /back|rear|environment|sau|trás|traseira/i.test(camera.label),
      );
      const camera = rearCamera || cameras[cameras.length - 1];
      const scanner = new Html5Qrcode("qr-reader", { verbose: false });
      scannerRef.current = scanner;
      setScannerActive(true);
      await scanner.start(
        camera.id || { facingMode: { ideal: "environment" } },
        {
          fps: 10,
          qrbox: (w, h) => {
            const edge = Math.max(160, Math.min(280, Math.min(w, h) - 48));
      ...
      scannerRef.current = null;
      if (failedScanner) {
        try { await failedScanner.stop(); } catch { /* scanner was not running */ }
        try { failedScanner.clear(); } catch { /* scanner was not initialized */ }
      }
      setScannerActive(false);
      const name = error instanceof DOMException ? error.name : "";
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        toast.error("Camera đang bị chặn. Hãy cấp quyền Camera cho trang này trong cài đặt trình duyệt rồi thử lại.");
      } else if (name === "NotFoundError" || name === "DevicesNotFoundError") {
        toast.error("Không tìm thấy camera trên thiết bị này. Bạn có thể chọn ảnh QR từ thư viện.");
      } else if (name === "NotReadableError" || name === "TrackStartError") {
        toast.error("Camera đang được ứng dụng khác sử dụng. Hãy đóng ứng dụng camera rồi thử lại.");
      } else {
        toast.error("Không mở được camera. Trên iPhone, hãy mở trang bằng Safari; hoặc chọn ảnh QR từ thư viện.");
      }
    }
  }
  ...
  async function scanFile(file?: File) {
    ...
    try {
      const { Html5Qrcode } = await import("html5-qrcode");
      scanner = new Html5Qrcode("qr-file-reader", { verbose: false });
      const decoded = await scanner.scanFile(file, true);
      await lookup(decoded);
    } catch { toast.error("Không đọc được QR trong ảnh. Hãy chọn ảnh rõ, đầy đủ bốn góc mã hoặc nhập mã voucher."); }
    finally {
      try { scanner?.clear(); } catch { /* scanner was already cleared */ }
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="p-4 sm:p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              <Button variant="outline" className="h-12" onClick={openImageLibrary}><ImagePlus /> Chọn từ thư viện</Button>
              <Button variant="outline" className="col-span-2 h-12 sm:col-span-1" onClick={() => setManualOpen(true)}><Keyboard /> Nhập mã</Button>
              <input ref={fileRef} className="hidden" type="file" accept="image/*" onChange={(e) => scanFile(e.target.files?.[0])} />
            </div>
            <p className="mt-3 text-center text-xs leading-5 text-slate-400">iPhone: nên mở bằng Safari. Android: nên mở bằng Chrome. Bạn cũng có thể chọn ảnh chụp màn hình QR từ thư viện.</p>
          </div>
        </section>
