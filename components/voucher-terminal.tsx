...
type Voucher = {
  ...
  status?: string; discount?: string | number; difficulty?: string;
  issued_at?: string; expires_at?: string; redeemed_at?: string;
};
...
function parseQr(raw: string) {
  ...
  try {
    ...
    return { query: url.searchParams.get("v") || value, qrToken: url.searchParams.get("t") || "" };
  } catch {
    return { query: value, qrToken: "" };
  }
}
...
export default function VoucherTerminal() {
  ...
  async function lookup(raw: string) {
    ...
    setBusy(true); setVoucher(null); setQrToken(parsed.qrToken);
    try {
      const token = sessionStorage.getItem(TOKEN_KEY);
      const r = await callApi({ action: "lookupVoucher", adminToken: token, query: parsed.query });
  ...
  async function redeem() {
    ...
    try {
      ...
      const r = await callApi({
        ...
        qrToken, redemptionPoint: "Touch to Play Staff Web",
      ...
      setVoucher({ ...voucher, status: "REDEEMED", redeemed_at: new Date().toISOString() });
            ...
            {!voucher ? <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-5 py-10 text-center"><TicketCheck className="mx-auto size-9 text-slate-300" /><p className="mt-3 text-sm font-medium text-slate-500">Chưa có voucher</p><p className="mt-1 text-xs text-slate-400">Quét QR hoặc nhập mã để tra cứu.</p></div> :
              <div className="mt-5">
                ...
                <dl className="mt-5 divide-y divide-slate-100 rounded-2xl bg-slate-50 px-4 text-sm">
                  <div className="flex justify-between py-3"><dt className="text-slate-500">Giá trị ưu đãi</dt><dd className="font-bold text-slate-800">{voucher.discount ? `${voucher.discount}${String(voucher.discount).includes("%") ? "" : "%"}` : "—"}</dd></div>
                  ...
                  <div className="flex justify-between gap-3 py-3"><dt className="text-slate-500">Hết hạn</dt><dd className="text-right font-semibold">{voucher.expires_at || "—"}</dd></div>
                </dl>
